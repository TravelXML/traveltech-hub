...
import { getMyJobs, resubmitJob } from '../services/jobService.js'
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h1 className="font-display text-3xl font-bold text-slate-900">My job postings</h1>
        <Link
          to="/add-job"
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex shrink-0 flex-wrap gap-2">
                  {item.status === 'approved' && (
                    <Link
                      to={`/dashboard/jobs/${item.id}/applicants`}
                  ...
                  {EDITABLE_STATUSES.includes(item.status) && (
                    <Link
                      to={`/dashboard/jobs/${item.id}/edit`}
