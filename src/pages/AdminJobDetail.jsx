...
import { getJobByIdForAdmin, approveJob, rejectJob, archiveJob } from '../services/adminService.js'
...
export default function AdminJobDetail() {
  ...
  async function handleReject(e) {
    ...
    await runAction(() => rejectJob(id, rejectReason.trim()), 'Reject this job?')
  ...
  if (!item) {
    return (
      <div className="mx-auto max-w-lg px-4 py-20 text-center">
        ...
        <Link to="/admin/jobs" className="mt-4 inline-block font-medium text-brand-600 hover:text-brand-700">
          Back to all jobs
        </Link>
      </div>
    )
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
      <Link to="/admin/jobs" className="text-sm font-medium text-slate-500 hover:text-slate-700">
        ← Back to all jobs
      </Link>
      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {item.listing?.name ?? 'Unknown company'} · {item.category} · {item.employmentType}
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {item.status === 'pending' && (
          <>
            <button
              disabled={busy}
              onClick={() =>
                runAction(() => approveJob(id), `Approve "${item.title}"? It will become publicly visible.`)
              }
        ...
        {item.status !== 'archived' && (
          <button
            disabled={busy}
            onClick={() =>
              runAction(() => archiveJob(id), `Archive "${item.title}"? It will be removed from public view.`)
            }
          ...
          <div>
            ...
            <dl className="mt-2 space-y-1 text-sm text-slate-700">
              <div>
                ...
                <dd className="inline">{item.remote ? 'Remote' : item.location}</dd>
              </div>
              <div>
                <dt className="inline text-slate-500">Experience: </dt>
                <dd className="inline">{item.experienceLevel || '—'}</dd>
              </div>
              <div>
                <dt className="inline text-slate-500">Salary: </dt>
                <dd className="inline">{item.salaryRange || '—'}</dd>
              </div>
              <div>
                <dt className="inline text-slate-500">Closes: </dt>
                <dd className="inline">{item.closesAt || 'Open until filled'}</dd>
              </div>
            </dl>
          </div>
        </section>
